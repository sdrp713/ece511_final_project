...
struct sbi_trap_regs* handle_interrupt(struct edge_syscall* syscall_data_ptr, size_t data_len, enclave_id eid){
  ...
  struct sbi_trap_regs* enclave_regs;
  int ret = stop_enclave(enclave_regs, syscall_data_ptr->syscall_num,  eid);
  if (syscall_data_ptr->syscall_num == HOST_INT) {
    ocall(2, syscall_data_ptr->data, strlen(syscall_data_ptr->data)+1, nullptr ,sizeof(char));
  }
  if (ret != 0) {
    return -1;
  }
